...
def clean_dupes(args):
        ...
        if file.is_file():
            try:
                ...
                if file_hash in seen_hashes:
                    ...
                    target = trash_dir / file.name
                    counter = 1
                    while target.exists():  # handle name collisions
                        target = trash_dir / f"{file.stem}_{counter}{file.suffix}"
                        counter += 1
                    if not args.d:
                        shutil.move(str(file), str(target))
                    print(f"Duplicate found: {file} (moved to {target})")
                    removed += 1
                else:
    ...
    print(f"Removed (moved) {removed} duplicates.")
    ...
    for file in src_dir.glob("*.ARW"):
        try:
            with rawpy.imread(str(file)) as raw:
                rgb = raw.postprocess()
            dest_file = dest_dir / (file.stem + ".jpg")
            imageio.imsave(dest_file, rgb)
...
def convert_arw_to_dng(args):
    ...
    dng_converter_path = (
        ...
        else "/Applications/Adobe DNG Converter.app/Contents/MacOS/Adobe DNG Converter"
    )
    if not Path(dng_converter_path).exists():
        print(
            f"Adobe DNG Converter not found at {dng_converter_path}, install it first."
        )
        return

    for arw_file in src_dir.glob("*.ARW"):
        try:
            subprocess.run(
                [
                    str(dng_converter_path),
                    "-d",
                    str(dest_dir),
                    "-c",  # keep original
                    str(arw_file),
            ...
            print(f"Converted {arw_file} -> {dest_dir}")
        except subprocess.CalledProcessError as e:
            print(f"Failed to convert {arw_file}: {e}")
...
def check_aspect_ratio(args):
    print(
        f"Checking aspect ratio in directory: {args.src_dir} for aspect ratio: {args.aspect_ratio}"
    )
    ...
    for photo in photos_data:
        if "ImageWidth" in photo and "ImageHeight" in photo:
            ...
            if (w, h) == (target_w, target_h) and (args.all or args.match):
                print(
                    f"Photo {photo['SourceFile']} matches aspect ratio {args.aspect_ratio}"
                )
            elif (w, h) != (target_w, target_h) and (args.all or args.not_match):
                print(
                    f"Photo {photo['SourceFile']} does not match aspect ratio {args.aspect_ratio} (actual: {w}:{h})"
                )
        else:
            print(f"Photo {photo['SourceFile']} does not have width/height data")


def get_by_aspect_ratio(args):
    print(
        f"Getting photos by aspect ratio {args.aspect_ratio} in directory: {args.src_dir}"
    )
    src_dir = Path(args.src_dir)
    target_w, target_h = map(int, args.aspect_ratio.split(":"))
    target_w, target_h = simplify_ratio(target_w, target_h)

    photos_data = get_exif_data(src_dir)
    matching = []
    ...
    for photo in photos_data:
        if "ImageWidth" in photo and "ImageHeight" in photo:
            ...
            if (w, h) == (target_w, target_h):
                matching.append(photo["SourceFile"])

    print(f"Found {len(matching)} photos with aspect ratio {args.aspect_ratio}:")
    for m in matching:
        print(m)
...
def main():
    parser = argparse.ArgumentParser(
        description="Photography utilities command line tool"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    # Sub-command: clean_dupes
    parser_something = subparsers.add_parser(
        "clean_dupes", help="Clean duplicate photos"
    )
    parser_something.add_argument(
        "-src_dir", required=True, help="Source directory containing photos"
    )
    group = parser_something.add_mutually_exclusive_group()
    group.add_argument(
        "--d", action="store_true", help="Dryrun mode, do not delete files"
    )
    parser_something.set_defaults(func=clean_dupes)

    # Sub-command: arw2jpg
    parser_something_else = subparsers.add_parser(
        "arw2jpg", help="Convert ARW files to JPG"
    )
    parser_something_else.add_argument(
        "-src_dir", required=True, help="Source directory containing ARW files"
    )
    parser_something_else.add_argument(
        "-dest_dir", required=True, help="Destination directory for converted JPG files"
    )
    parser_something_else.set_defaults(func=convert_arw_to_jpg)

    # Sub-command: arw2dng
    parser_something_else = subparsers.add_parser(
        "arw2dng", help="Convert ARW files to DNG"
    )
    parser_something_else.add_argument(
        "-src_dir", required=True, help="Source directory containing ARW files"
    )
    ...
    parser_something_else.add_argument(
        "-converter_path",
        required=False,
        help="Destination of Adobe DNG Converter, default is /Applications/Adobe DNG Converter.app/Contents/MacOS/Adobe DNG Converter",
    )
    parser_something_else.set_defaults(func=convert_arw_to_dng)

    # Sub-command: exif2json
    parser_something_else = subparsers.add_parser(
        "exif2json", help="Convert EXIF data to JSON"
    )
    parser_something_else.add_argument(
        "-src_dir", required=True, help="Source directory containing photos"
    )
    parser_something_else.add_argument(
        "-dest_file", required=True, help="Destination JSON file for EXIF data"
    )
    parser_something_else.set_defaults(func=export_exif_to_json)

    # Sub-command: organize
    parser_something_else = subparsers.add_parser(
        "organize", help="Organize photos by date"
    )
    parser_something_else.add_argument(
        "-src_dir", required=True, help="Source directory containing photos"
    )
    parser_something_else.add_argument(
        "-dest_dir", required=True, help="Destination directory for organized photos"
    )
    parser_something_else.set_defaults(func=organize_photos)

    # Sub-command: check_aspect_ratio
    parser_something_else = subparsers.add_parser(
        "check_aspect_ratio", help="Check aspect ratio of photos"
    )
    parser_something_else.add_argument(
        "-src_dir", required=True, help="Source directory containing photos"
    )
    parser_something_else.add_argument(
        "-aspect_ratio", required=True, help="Aspect ratio to check (e.g., 16:9)"
    )
    group = parser_something_else.add_mutually_exclusive_group()
    group.add_argument("--all", action="store_true", help="Print all photos")
    group.add_argument(
        "--match", action="store_true", help="Print only matching photos"
    )
    group.add_argument(
        "--not_match", action="store_true", help="Print only non-matching photos"
    )
    parser_something_else.set_defaults(func=check_aspect_ratio)

    # Sub-command: get_by_aspect_ratio
    parser_something_else = subparsers.add_parser(
        "get_by_aspect_ratio", help="Get photos by aspect ratio"
    )
    parser_something_else.add_argument(
        "-src_dir", required=True, help="Source directory containing photos"
    )
    parser_something_else.add_argument(
        "-aspect_ratio", required=True, help="Aspect ratio to filter by (e.g., 16:9)"
    )
